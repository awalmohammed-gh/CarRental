...
const CarsDetails = () => {
  const { id } = useParams();
  const { setAddCar, rentals, setRentals } = useCarRental();
  ...
  const handleRentNow = () => {
    if (carsData.status === "Sell") {
      setAddCar((prev) =>
        prev.includes(carsData.id) ? prev : [...prev, carsData.id]
      );
      setShowSuccess(true);
      ...
      setRentals((prev) => [
        ...prev,
        {
          id: Date.now(),
          carId: carsData.id,
          carName: carsData.name,
          carImage: carsData.images?.[0],
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link
            to="/cars"
            className="mt-4 inline-block text-[#9810fa] hover:underline"
          >
            Back to Cars
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {(showSuccess || rentalSuccess) && (
        ...
        >
          ...
          <span>
            {showSuccess 
              ? `${carsData.name} added to cart!` 
              : `Rental request sent for ${carsData.name}!`}
          </span>
        </motion.div>
      )}
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-3xl">
          ...
          >
            ...
            <p className="text-base md:text-lg text-gray-200 max-w-2xl mx-auto">
              {carsData.brand} {carsData.model} • {carsData.year} •{" "}
              {carsData.category}
            </p>
            <div className="flex items-center justify-center gap-4 mt-4">
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${carsData.availability ? "bg-green-500" : "bg-red-500"}`}
                ></div>
                <span className="text-sm">
                  {carsData.availability
                    ? "Available Now"
      ...
      <div className="container mx-auto px-4 max-w-7xl py-8 md:py-12">
        ...
        >
          <Link
            to="/"
            className="hover:text-[#9810fa] transition-colors flex items-center gap-1"
          >
            <Car className="w-4 h-4" />
            Home
          </Link>
          <span>/</span>
          <Link to="/cars" className="hover:text-[#9810fa] transition-colors">
            Cars
          </Link>
          <span>/</span>
          ...
          >
            <div className="sticky top-24">
              ...
              <div className="relative bg-white rounded-2xl overflow-hidden shadow-lg mb-4">
                ...
                {/* Status Badge */}
                <div
                  className={`absolute top-4 left-4 px-4 py-2 rounded-lg text-white font-semibold text-sm ${
          ...
          >
            ...
            <div className="mb-6 p-4 bg-white rounded-xl shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-700">
                  Location:{" "}
                  <span className="font-semibold">{carsData.location}</span>
                </span>
              </div>
            </div>

            {/* Rental Date Selection (Only for Rent) */}
            {carsData.status === "Rent" && (
              <div className="mb-6 p-4 bg-white rounded-xl shadow-sm">
                ...
                <input
                  ...
                  className="w-full p-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#9810fa] focus:border-transparent transition-all"
                  required
                />
                {startDate && (
                  <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
                    <AlertCircle className="w-3 h-3" />
                    Rental period: 24 hours from selected start time
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex gap-4">
              <button
                ...
                disabled={carsData.status === "Rent" && !startDate}
                className={`flex-1 py-3.5 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl ${
                  carsData.status === "Rent" && !startDate
                    ? "bg-gray-300 cursor-not-allowed text-gray-500"
                    : "bg-[#9810fa] hover:bg-[#7a0cc8] text-white"
                }`}
              >
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-shrink-0">
                    <div className="w-24 h-24 bg-gradient-to-br from-[#9810fa] to-[#7a0cc8] rounded-full flex items-center justify-center">
                      {carsData.dealer.image ? (
                        <img
                          src={carsData.dealer.image}
                          alt={carsData.dealer.name}
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-gray-800 mb-2">
                      {carsData.dealer.name}
                    </h3>
                    <div className="flex items-center gap-2 mb-3">
                      <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                      <span className="text-sm font-semibold">
                        {carsData.dealer.rating}
                      </span>
                      <span className="text-xs text-gray-500">
                        • {carsData.dealer.yearsInBusiness} years in business
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      <a
                        href={`mailto:${carsData.dealer.email}`}
                        className="flex items-center gap-2 px-4 py-2 bg-[#9810fa] text-white rounded-lg hover:bg-[#7a0cc8] transition-all"
                      >
                        <Mail className="w-4 h-4" />
                        Message Dealer
                      </a>
                      <a
                        href={`https://wa.me/${carsData.dealer.phone.replace(/\D/g, '')}`}
                        target="_blank"
                      ...
                      <a
                        href={`tel:${carsData.dealer.phone}`}
                        className="flex items-center gap-2 px-4 py-2 border border-[#9810fa] text-[#9810fa] rounded-lg hover:bg-[#9810fa] hover:text-white transition-all"
                      >
